...
* 用一台服务器为多位客户服务，通过不同的域名映射相同的IP实现
* 需要在 host 首部完整指定主机名或者域名的 URI

## 通信数据转发程序：代理、网关、隧道

* 用于通信数据转发的应用程序，配合服务器完成工作
* 可以将请求转发给通信线路上的下一站服务器，且能接收下一站服务器发过来的响应以转发给客户端
* 网关
  * 转发其他服务器通信数据的服务器，接收客户端发过来的请求时，并对请求进行处理
* 隧道
  * 在相隔较远的客户端和服务端之间进行转发，并保持双方的通信连接

### 代理

* 转发功能。接收客户端发送的请求并转发给服务器，同时接收服务器的响应并转发给客户端
* 不改变请求的 URI，直接发送给前方持有资源的目标服务器（源服务器）
* 源服务器处理后返回响应给代理服务器，代理服务器将结果转发给客户端
* 可以使用多台代理服务器。转发时，需要添加 Via 首部字段以标记经过的主机
* 作用
  * 利用缓存减少网络带宽的流量
  * 针对特定网站的访问控制
  * 等
* 缓存代理
  * 会预先将资源的副本保存到代理服务器上
  * 请求时，代理服务器先检查资源是否存在代理服务器上。如果有，则直接返回，不需要再请求源服务器
* 透明代理
  * 不对报文做任何加工的代理
...
* 工作机制和代理类似
* 可以提高通信的安全性。可以在客户端和网关之间的通信线路上加密以确保连接的安全性

### 隧道

* 可以按要求建立起一条与其他服务器的通信线路
* 目的是确保客户端和服务器之间进行安全的通信，如使用 SSL 等加密手段
* 隧道本身不解析HTTP请求，按照原样中转给之后的服务器，在通信双方断开连接时结束

## 保存资源的缓存

* 缓存是指代理服务器或者客户端保存资源副本，以减少源服务器的压力、节省通信的流量和通信时间
* 缓存服务器是代理服务器的一种

### 缓存有效期

* 根据客户端的要求和缓存的有效期，向源服务器确认资源的有效性，以判断是否需要更新缓存的资源

### 客户端缓存

* 客户端也可以缓存资源，并判断资源是否过期

# HTTP 首部

* HTTP 协议的请求和响应报文必定包含 HTTP 首部
...
```javascript
首部字段名: 字段值
Content-type: text/html  // 如果重复，则优先处理第一次出现的首部字段
...
* HTTP 本身不具备加密功能，无法做到对整个通信整体进行加密，使用明文方式发送数据
* TCP/IP 是可能被窃听的网络，通过抓包可以获取HTTP请求和响应的内容
* 加密方式以防止被窃听
  * 通信的加密：通过和 SSL 或者 TLS 的组合使用，加密通信内容
  * 使用 SSL 建立安全通信线路。与 SSL 组合使用的 HTTP 即为 HTTPS，超文本传输安全协议
  * 内容的加密：对通信内容本身进行加密，然后再发送请求。需要两端具备加密和解密机制

### 不验证通信方身份，可能遭遇伪装

* 不会验证返回的响应是否来自真正的服务器
* 不会验证返回的响应是否发送到了真正的客户端
* 查明对手的证书
  * 使用 SSL可以确定通信方，不仅提供加密处理，还提供了证书以确定对方
  * 证书：由第三个颁发，以证明服务端和客户端是实际存在的。且难以伪造

### 无法验证报文的完整性，有可能已遭篡改

* 完整性：即信息的准确度。可能被中间人攻击，拦截并篡改
* 可以使用 MD5 和 SHA-1 、数字签名方法
* 但是以上，并不方便。需要用户即开发者亲自检查验证数据是否完整，浏览器无法自动完成
* 又但是，以上都可能会被篡改，也无法保证绝对安全，有必要使用 HTTPS

## HTTPS

* HTTP + 加密 + 认证 + 完整性保护
* HTTP 加上加密处理、认证以及完整性保护
* 如在 HTTP 中，当用户输入信用卡号，当通信线路被窃听，该信用卡号即遭到泄露
* 是身披 SSL 的 HTTP
* 通常，HTTP 直接 TCP 通信。当使用 HTTPS时，则先和 SSL 通信，再由 SSL 和 TCP 通信

### 相互交换密钥的公钥加密技术

* SSL：公钥加密的处理方式
* 加密算法公开，密钥保密，以确保加密方法的安全
* 加密和解密用同一个密钥，为共享密钥加密，即对称密钥加密。密钥也无法安全发送给对方
* 公钥加密为非对称加密方式，公钥可以随意发布，私钥自己保存
* 过程：发送密文的一端使用对方的**公钥加密**，对方使用自己的**私钥解密**
* 不必担心密钥被窃走，且密文难以被破解
* HTTPS 比 HTTP 慢 2 - 100 倍
* HTTPS 加密通信会消耗更多的 CPU 及内存资源
* 证书需要从可信的 CA 购买
* 略