* 源代码参见：`JVM-code`，资料参见《实战JAVA虚拟机 JVM故障诊断与性能优化》、《深入理解JAVA虚拟机》
...
* 无法处理循环引用的情况（**JDK1.1之后，这个算法已经不再使用**）
...
* 在 JAVA 虚拟机中，每个对象都一个对象头，以保存对象的系统消息。其中`Mark Word`数据区是实现锁的关键
* `Mark Word` 数据区可以存放对象的哈希值、对象年龄、锁的指针等信息
* 一个对象是否占用锁，占有哪个锁，都记录在`Mark Word`数据区中

## 虚拟机中锁的实现和优化

### 偏向锁

* 若程序没有竞争，则取消之前已经取得的锁的线程同步操作；即若某一个锁被线程获取后，便进入偏向模式，当该线程再次请求该锁时，无需再进行同步操作，节省了时间；在此过程中，如果有其他线程进行了锁的请求，则锁退出偏向模式

```bash
-XX:+UseBiasedLocking  # 启用偏向锁
```

* 当锁处于偏向模式时，对象头会记录获得锁的线程；当该线程再次尝试获得锁时，通过`Mark Word`数据区的线程信息就可以判断当前线程是否持有偏向锁
* 不适合于激烈竞争的场合，因为锁难以一直保持偏向模式

### 轻量级锁

* 若偏向锁失败，虚拟机将让线程申请轻量级锁
